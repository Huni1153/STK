...
//		addListener();
	...
	class TopListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e) {
			...
			else if(e.getSource() == tFormat)
			{
				//System.out.println("정윤 들여쓰기 시작~!");
				AutoFormatting format = new AutoFormatting();
				format.check();
			}
...
//////////0531_2015 대섭 슈졍
				...
				StringReader sr11 = new StringReader(leftArea.getText()); 
				StringReader sr22=new StringReader(rightArea.getText());
				BufferedReader leftBr = new BufferedReader(sr11);
				BufferedReader rightBr=new BufferedReader(sr22);
...
//////////0531_2015 대섭 슈졍
					...
					{
						DocumentSaveAsUI saveAs=new DocumentSaveAsUI(currentDocCode,(String)currentDocumentArr[2],left,right,leftList,rightList);
						if(saveAs.set())
						{
							dispose();
						}
					...
					else
					{
						...
						save.createJava((String)currentDocumentArr[1], leftList, rightList, ".java");
			// 0531_2150 세훈 수정
...
////// 0529_0421 대섭////// 0529_0421 대섭 끝
	...
	private void setTopButtons()
	{
		...
		tFormat.addActionListener(topListener);
...
// 0601 정윤
	class AutoFormatting
	{
		public void check()
		{
         ...
         BufferedReader reader = new BufferedReader(new StringReader(leftArea.getText()));
         while(true)
         {
            try
            {
               ...
               if(str == null)
                  break;
               System.out.println("코드 : "+str);
               //System.out.println("trim 코드 : "+str.trim());
            }
            catch (IOException ioe)
            {
            }
         }
         System.out.println("끄읕1");*/
			BufferedReader reader2 = new BufferedReader(new StringReader(leftArea.getText()));
         ...
         //System.out.println("끄읕2");
         BufferedReader reader3 = new BufferedReader(new StringReader(strBuffer.toString()));
         //여기부터 로직 시작~!
         int j=0;
         int count=0;//탭갯수
         ...
         while(true)
         {
            //System.out.println("count : "+count);//확인
            String str = null;
            try
            {
               ...
               String temperary = reader3.readLine();
               if(temperary == null)
                  break;
               temp.append(temperary);
               str = temp.toString();
              // System.out.println("tab코드 : "+str);//확인
            ...
            {
               if((""+str.charAt(j)).equals("{")==true)
               {
                  count++;
                  break;   
               }
               else if((""+str.charAt(j)).equals("}")==true)
               {
                  if(str.length() != 1)// && (str.charAt(str.length()-1)+"").equals('\t'))
                     str = str.substring(1);
                  count--;
                  break;   
               }
         ...
         //확인
         //System.out.println("최종확인");
         BufferedReader reader4 = new BufferedReader(new StringReader(strBuffer2.toString()));
         while(true)
         {
            try
            {
         ...
         //실제 ui에 뿌리기!!!!!!!!!!!!ㅎ..ㅎ
         //leftArea.setText("");//다지우기
         leftArea.setText(strBuffer2.toString());
      }
   }
